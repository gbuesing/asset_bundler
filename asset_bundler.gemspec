# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run the gemspec command
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{asset_bundler}
  s.version = "0.2.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Geoff Buesing"]
  s.date = %q{2010-06-24}
  s.description = %q{Adapted from asset bundling functionality in ActionPack.}
  s.email = %q{gbuesing@gmail.com}
  s.extra_rdoc_files = [
    "README.md"
  ]
  s.files = [
    ".gitignore",
     "README.md",
     "Rakefile",
     "VERSION",
     "lib/asset_bundler.rb",
     "test/fixtures/.DS_Store",
     "test/fixtures/public/scripts/one.js",
     "test/fixtures/public/scripts/three.js",
     "test/fixtures/public/scripts/two.js",
     "test/fixtures/public/styles/one.css",
     "test/fixtures/public/styles/three.css",
     "test/fixtures/public/styles/two.css",
     "test/test_asset_bundler.rb"
  ]
  s.homepage = %q{http://github.com/gbuesing/asset_bundler}
  s.rdoc_options = ["--charset=UTF-8"]
  s.require_paths = ["lib"]
  s.rubygems_version = %q{1.3.7}
  s.summary = %q{A simple asset bundling solution.}
  s.test_files = [
    "test/test_asset_bundler.rb"
  ]

  if s.respond_to? :specification_version then
    current_version = Gem::Specification::CURRENT_SPECIFICATION_VERSION
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<asset_timestamps_cache>, [">= 0.1.1"])
    else
      s.add_dependency(%q<asset_timestamps_cache>, [">= 0.1.1"])
    end
  else
    s.add_dependency(%q<asset_timestamps_cache>, [">= 0.1.1"])
  end
end

